        ...
        <>
            ...
            <table>
                ...
                <tr>
                    ...
                    {currentPage[0]?.variant && currentPage[0].levelRequired && title !== 'Unique Armours' ? <th onClick={()=>appDispatch({type:'SET_SORT_VARIANT'})} className={'dif_item'}>Passives</th>:null}
                    {currentPage[0]?.mapTier && currentPage[0].baseType ? <th onClick={()=>appDispatch({type:'SET_SORT_MAP_TIER'})} className={'dif_item'}>Map Tier</th>:null}
                    {currentPage[0]?.levelRequired ? <th onClick={()=>appDispatch({type:'SET_SORT_VALUE'})} className={'dif_item'}>Level</th>:null}
                    {currentPage[0]?.stackSize && currentPage[0]?.artFilename ? <th onClick={()=>appDispatch({type:'SET_SORT_STACK_SIZE'})} className={'dif_item'}>Stack Size</th>:null}
                    ...
                    return (
                        <tbody key={id} className={'form1_li'}>
                            <tr>
                                ...
                                {currentPage[0]?.variant && currentPage[0].levelRequired && title !== 'Unique Armours' ? <td className={'dif_item'}>{variant?.slice(0,2)}</td>:null}
